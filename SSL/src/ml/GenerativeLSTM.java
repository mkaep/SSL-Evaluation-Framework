...
public class GenerativeLSTM implements Approach {
	...
	public void createInputFiles(XLog originalLog, Set<ReducedLogContainer> reducedLogs, String path, boolean lifecycle) {
		Parser p = new Parser();
		long maxDuration = p.getMaximalDuration(originalLog);
		...
		for(ReducedLogContainer log : reducedLogs) {
			...
			try {
			    ...
			    out.print("caseid,task,user,end_timestamp,role,dur,ac_index,rl_index,dur_norm"+"\n");
				...
				while(logIterator.hasNext()) {
					...
					//First timestamp of the frist event
					Date previousTimestamp = XTimeExtension.instance().extractTimestamp(currentTrace.get(0));
					while(traceIterator.hasNext()) {
						...
						Date nextTimestamp = XTimeExtension.instance().extractTimestamp(currentEvent);
						
						String role = originatorRoleMap.get(originator);

						long tbtw = getDuration(previousTimestamp, nextTimestamp);
						String duration = String.valueOf(tbtw);
						
						
						int aIndex = activityIndex.get(activity).intValue();
						int rIndex = roleIndex.get(role).intValue();
						
						String durNorm = String.valueOf(Math.log1p(tbtw)/Math.log(maxDuration));

						line = line+caseId+","+activity+","+originator+","+nextTimestamp.toString()+","+role+","+duration+","+aIndex+","+rIndex+","+durNorm+"\n";
						out.print(line);
	...
	private long getDuration(Date previousTimestamp, Date nextTimestamp) {
		return (nextTimestamp.getTime()-previousTimestamp.getTime())/1000;
	}
